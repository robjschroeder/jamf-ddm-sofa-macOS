...
ddmEnforcedInstallDate=$( awk -v date=$(date -v-"$numberOfDays"d +%Y-%m-%d) '$1 >= date' /var/log/install.log | grep EnforcedInstallDate | sed -n 's/.*EnforcedInstallDate:\([^|]*\)|VersionString:\([^|]*\)|BuildVersionString:\([^|]*\).*/\1 macOS \2 (\3)/p' | sort -u )
